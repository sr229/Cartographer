...
app.post('/cartographer-webhook', async (req, res) => {
    ...
    tree = tree.filter(v => v !== SITEMAP_PATH && v.startsWith(SITEMAP_GEN_PATH) && !IGNORE_PATHS.map(end => v.includes(end)).reduce((m, va) => m || va)).reduce((m, val) => {
        ...
        if (val.endsWith('/') && val.split('/').length === 2 && !m[val.slice(0, -1)]) return Object.assign(m, {[val.slice(0, -1)]: {}});
...
function genContent(tree) {
    ...
    */
    ...
    return Object.entries(tree).reduce((m, [path, children]) => {
        ...
        if (!Object.keys(children).length) return m.concat(`- [${path.replace(/_/g, ' ')}](${path.replace(/\s+/g, '_')})`);

        // Make a new array with the path name.
        let thisTree = [`- [${path.replace(/_/g, ' ')}](${path.replace(/\s+/g, '_')})`];
        ...
        (function looper(subTree, indent, fullPath) {
            for (let child of Object.entries(subTree)) {
                thisTree.push(`${' '.repeat(indent + 1)}- [${child[0].replace(/_/g, ' ')}](${(`${fullPath}/${child[0]}`).replace(/\s+/g, '_')})`);
